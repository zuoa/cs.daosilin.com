        ...
        try:
            ...
            for player_id in player_ids:
                try:
                    ...
                    # 使用智能称号选择
                    best_titles = self.title_system.get_smart_titles(player_data)
                    ...
                    if best_titles:
                        ...
                        if PlayerTitle.update_player_titles(player_id, cup_name, play_day, titles_data):
                            success_count += 1
                            logger.info(f"成功为玩家 {player_id} 计算并保存称号")
                        else:
    ...
    def recalculate_titles_for_player(self, player_id: str, cup_name: str, play_day: str = None) -> bool:
        ...
        try:
            ...
            if best_titles:
                ...
                # 保存到数据库
                if PlayerTitle.update_player_titles(player_id, cup_name, play_day, titles_data):
                    logger.info(f"成功重新计算玩家 {player_id} 的称号")
                    return True
                else:
                    logger.error(f"保存玩家 {player_id} 称号失败")
                    return False
    ...
    def get_title_distribution_stats(self, cup_name: str, play_day: str = None) -> Dict:
        ...
        try:
            ...
            stats = {
                ...
                'title_rarity_distribution': {'common': 0, 'rare': 0, 'epic': 0, 'legendary': 0},
            ...
            for player_id, titles in all_titles.items():
                if titles:
                    ...
                    for title in titles:
                        ...
                        if rarity in stats['title_rarity_distribution']:
                            stats['title_rarity_distribution'][rarity] += 1
